...
{
    public static class JSONTournamentImporter
    {
        public static Tournament CreateNewTournament(string jsonString)
        {
            ...
            dynamic rawJson = JsonConvert.DeserializeObject(jsonString)!;

            Tournament tournament = new Tournament()
            {
                Name = jsonTourn.name!,
                Year = jsonTourn.start!.Substring(0, 4),
                RawJsonTournament = rawJson
            };
            ...
            foreach (Tuple<Event, JSONRound> round in roundsToProcess)
            {
                ...
                foreach (JSONSection jsonSection in jsonRound.sections)
                {
                    Debate debate = new Debate()
                    {
                        ...
                        RawJsonBallots = FindRawJsonBallots(tournament.RawJsonTournament, jsonSection.id!)
                    };
        ...
        private static List<JObject> FindRawJsonBallots(dynamic rawJsonTournament, string sectionId)
        {
            List<JObject> ballots = new();

            foreach (var category in rawJsonTournament.categories)
            {
                foreach (var @event in category.events)
                {
                    foreach (var round in @event.rounds)
                    {
                        ...
                        {
                            if (section.id.Value.Equals(sectionId))
                            {
                                foreach(var ballot in section.ballots)
                                {
                                    JObject jBallot = (JObject)ballot;
                                    ballots.Add(jBallot);
                                }
                            }
                        }
